...
2. Fetch branches. When you open the branch picker the plugin asks GitHub for branch names, falling back to the repository’s default branch if you do not pick one. Use **Refresh** any time you need the latest list, or choose **Create new…** to spin up a branch without leaving Figma—the plugin creates it from the default branch and switches your export over immediately.【F:src/app/ui.html†L1120-L1126】【F:src/core/github/api.ts†L367-L444】 When you later enable the pull-request toggle, the plugin targets that same default branch as the base and requires your export branch to be different, so create or select a feature branch before opening the PR flow.【F:src/app/github/ui.ts†L865-L927】【F:src/app/github/dispatcher.ts†L661-L711】
...
- Need a clean branch for review? Use **Create new…** in the branch section to stage one from the repo’s default branch, and the plugin will log a link to the fresh branch so you can share it immediately.【F:src/app/ui.html†L1120-L1126】【F:src/app/github/ui.ts†L1660-L1688】
