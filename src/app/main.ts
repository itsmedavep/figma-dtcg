...
type CollectionsDataPayload = Extract<PluginToUi, { type: 'COLLECTIONS_DATA' }>['payload'];
...
const pendingStyleEvents = new Map<string, { create?: boolean; update?: boolean; delete?: boolean; attempts?: number; loggedUpdate?: boolean }>();
const styleDisplayIds = new Map<string, string>();
const knownStyleIds = new Set<string>();

function computeCollectionsSignatureFromPayload(payload: CollectionsDataPayload): string {
  const parts: string[] = [];
...
async function refreshCollections(reason: RefreshReason, autoSource?: AutoRefreshSource | null): Promise<void> {
...
function isStyleDocumentChange(change: DocumentChange | null | undefined): boolean {
...
function recordStyleEvent(type: string | undefined, idRaw: string | undefined): void {
  if (!type) return;
...
function collectCurrentStyleIds(): Set<string> {
  ...
  try {
    const collect = (styles: BaseStyle[] | ReadonlyArray<BaseStyle> | undefined) => {
      if (!styles) return;
...
function flushStyleEventLogs(prev: Set<string>, next: Set<string>): void {
  ...
  const survivors = new Map<string, { create?: boolean; update?: boolean; delete?: boolean; attempts?: number }>();
...
function scheduleVariablePoll(): void {
