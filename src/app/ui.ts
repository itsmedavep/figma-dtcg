...
function setImportPreference(contexts: string[]): void {
    ...
    const same =
        importPreference &&
        contextsEqual(importPreference.contexts, normalized);
...
function renderImportLog(): void {
    ...
    for (let idx = importLogEntries.length - 1; idx >= 0; idx--) {
        ...
        if (
            ...
            importPreference &&
            importPreference.contexts.length > 0
        ) {
...
function openImportScopeModal(opts: {
    ...
    for (let i = 0; i < collections.length; i++) {
        ...
        for (let j = 0; j < modes.length; j++) {
            ...
            const list =
                importScopeModalState.inputsByCollection.get(collection) || [];
...
function requestPreviewForCurrent(): void {
    ...
    if (!collection || !mode) {
        if (w3cPreviewEl)
            w3cPreviewEl.textContent =
                "{ /* select a collection & mode to preview */ }";
        return;
    }
    const styleDictionary = !!(
        styleDictionaryChk && styleDictionaryChk.checked
    );
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (importScopeClearBtn) {
        importScopeClearBtn.addEventListener("click", () =>
    ...
    if (importScopeConfirmBtn) {
        importScopeConfirmBtn.addEventListener("click", () => {
            ...
            const remember = importScopeRememberChk
                ? !!importScopeRememberChk.checked
                : false;
            closeImportScopeModal();
    ...
    if (importScopeCancelBtn) {
        importScopeCancelBtn.addEventListener("click", () =>
            closeImportScopeModal()
        );
    }
    ...
    if (drawerToggleBtn) {
        drawerToggleBtn.addEventListener("click", () => {
            const current =
                drawerToggleBtn!.getAttribute("aria-expanded") === "true";
    ...
    if (copyRawBtn)
        copyRawBtn.addEventListener("click", () =>
            copyElText(
                document.getElementById("raw") as HTMLElement,
                "Raw Figma Collections"
            )
        );
    if (copyW3cBtn)
        copyW3cBtn.addEventListener("click", () =>
            copyElText(
                document.getElementById("w3cPreview") as HTMLElement,
                "W3C Preview"
            )
        );
    if (copyLogBtn)
