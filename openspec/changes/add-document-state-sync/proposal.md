...
The plugin currently snapshots styles and variables when the GitHub tab loads, but it does not notice subsequent document edits unless the user manually restarts the plugin. Designers often tweak color styles or token variables after opening the plugin—especially while preparing an export—and the GitHub payload then exports stale data. Figma exposes a `documentchange` API for styles but not for variables, so the plugin needs a hybrid strategy: event-driven refreshes for styles and a lightweight poller for variables. Without this, exported token graphs constantly lag behind the canvas, forcing teams to double-check every export manually.
...
-   Normalize both event-driven and polled updates into the existing collection cache so downstream GitHub exports/imports always read the most recent document snapshot.
-   Surface minimal diagnostic UX (e.g., a “Updating...” pulse) so users know when the plugin is refreshing and avoid spamming the UI with redundant redraws.
